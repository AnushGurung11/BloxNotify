...
// Rarities shown in the predictions even when their model score is low.
// Fruits players care most about rarely appear in the history, so they never
// crack the plain top-3; these picks surface them.
const PREFERRED_RARITIES = ['Legendary', 'Mythical'];
const PREFERRED_PICKS = 2;
 ...
 * @param {Object<string, string>} [rarities] fruit name -> rarity; when
 *   given, up to `PREFERRED_PICKS` Legendary/Mythical fruits are added after
 *   the top-3 so high-value fruits show up even with low model scores.
 * @returns {Array<{name: string, confidence: number}>} ranked candidates,
 *   confidence normalized over all candidates (0..1)
 */
function rankCandidates(stats, slot, currentFruits, rarities) {
  ...
  const picks = scores.slice(0, 3);
  const picked = new Set(picks.map((p) => p.name));
  if (rarities) {
    let added = 0;
    for (const candidate of scores) {
      if (added >= PREFERRED_PICKS) break;
      if (picked.has(candidate.name)) continue;
      const rarity = rarities[candidate.name];
      if (rarity && PREFERRED_RARITIES.includes(rarity)) {
 ...
 * @param {Object<string, string>} [deps.rarities] fruit name -> rarity
 * @param {Date} [deps.now] reference time (defaults to now)
 * @returns {{nextResetAt: number, predictions: Array<{name: string, confidence: number}>, rating: object}}
 */
function predict({ entries, currentFruits, rarities, now = new Date() }) {
  const stats = buildStats(entries);
  const slot = new Date(nextResetAt(now)).getUTCHours();
  return {
    nextResetAt: nextResetAt(now),
    predictions: rankCandidates(stats, slot, currentFruits, rarities),
    rating: backtest(entries),
  };
}

module.exports = { predict, backtest, rankCandidates, buildStats, addEntry, nextResetAt };