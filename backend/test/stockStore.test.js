...
describe('stockStore', () => {
  test('write then read round-trips fruits and sets updatedAt', () => {
    const file = path.join(tmpDir, 'last-known-stock.json');
    const record = writeStock({ fruits: ['Flame', 'Light'] }, file);
  ...
  test('readStock returns empty record for malformed JSON', () => {
    ...
    expect(readStock(file)).toEqual({ fruits: [], updatedAt: null });
  });
