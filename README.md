...
3. **Trade calculator** (`lib/screens/trade_screen.dart`) — simulates an in-game trade: pick items (live values from `GET /values`) for the "you give" and "you receive" sides from **compact picker lists** (small image with the name and value beside it; one tap adds the item, checkmarks show what is already selected). Selected items are shown as a **grid of fruit image tiles** (tap to remove); totals are compared and a **Win/Loss verdict with the exact amount gained or lost** updates in real time.
